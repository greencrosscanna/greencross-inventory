...
retired): `/gxbrain` reads notes addressed to `to_app=inventory`, resolves done ones (`resolve_note`), and
...
`deploy_version` endpoint + shared untracked `.gx_deploy_secret`); binds `GXCore` library **v153** (verified live via `?action=libversion`).
